...
void sort(receipt*& list, int count, const string& file_name);
...
void fill_struct_from_file(receipt* list, int* count, const string& file_name){
    ...
    while (!fin.eof()){
        auto temp = new receipt[*count + 1];
        for (int i = 0; i < *count; i++){
            temp[i] = list[i];
        }
        delete[] list;
        list = temp;
        fin.read((char*)&list[*count], sizeof(receipt));
        cout << list[*count].name << " " << list[*count].receipt_date << " " << list[*count].order_status << endl;
        (*count)++;
    }
    fin.close();
}
...
void rewrite_file(receipt*& list, int count, const string& file_name){
    ...
    for (int i = 0; i < count; ++i) {
        fout.write((char*)&list[i], sizeof(receipt));
    }
    fout.close();
}
